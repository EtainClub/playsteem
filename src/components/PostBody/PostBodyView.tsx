...
interface Props {
  ...
  openImageGallery: (show: boolean) => void;
}
const PostBodyView = (props: Props): JSX.Element => {
  //// props
  const { linkActionRef, imageActionRef, showGalleryModal, imagesUrl } = props;

  //// language
  const intl = useIntl();

  const { body, bodyFontSize } = props;
    ...
    <Block flex>
      <Modal visible={showGalleryModal} transparent={true}>
        <ImageViewer
          imageUrls={imagesUrl}
          enableSwipeDown
          onCancel={() => props.openImageGallery(false)}
          onClick={() => props.openImageGallery(false)}
        />
      </Modal>
      ...
      <ActionSheet ref={imageActionRef}>
        <Block center>
          <Button
            color="success"
            onPress={() => { props.openImageGallery(true); }}>
            {intl.formatMessage({ id: 'Actionsheet.view' })}
          </Button>
          <Button
            color="primary"
